...
use pdf_viewer_core::glyph_layout::{
    build_editor_session_text_plan, infer_run_advance, is_decorative_text, EditorSessionTextPlan,
};
use pdf_viewer_core::list_semantics::{derive_list_text_semantics, ListMarkerKind};
...
use crate::editor::source_geometry::source_visual_bbox_from_runs;
use crate::editor::source_runs::{
    original_paint_runs_for_target, resolve_preferred_editor_session,
};
use crate::editor::source_text::session_source_text;
use crate::editor::text_geometry::caret_index_at_page_point_with_plan;
...
fn resolve_shell_bbox(target_session: &EditorSession, split: &SessionSplit) -> BoundingBox {
    // For list paragraphs, the editor shell must be derived from the marker +
    // editable body only. Re-merging the original target session bbox pulls in
    // unrelated decorative paths (for example section dividers), which then
    // leak into edit mode as long blue horizontal bars.
...
fn normalize_template_run_for_draft(run: &LayoutRun) -> LayoutRun {
    ...
    // rhythm as the original PDF even after text edits.
    normalized.char_origins.clear();
    normalized.char_widths.clear();
    normalized.object_ids.clear();
    normalized.object_indices.clear();
...
fn build_plan_for_target_session(
    ...
    target: crate::editor::edit_target::EditorEditTarget,
    click_page_point: Option<(f32, f32)>,
) -> Option<EditorDocumentPlan> {
    ...
    // Opening the editor should use the exact same body-session hit-test path as
    // subsequent active-editor clicks so the initial caret lands where the user
    // actually clicked instead of snapping back to the start of the line.
