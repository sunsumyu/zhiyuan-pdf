use pdf_viewer_core::models::{BoundingBox, EditorSession, LayoutRun};

pub(crate) fn source_session_visual_bbox(session: &EditorSession) -> Option<BoundingBox> {
    source_visual_bbox_from_runs(&session.paragraph.runs)
}

pub(crate) fn source_visual_bbox_from_runs(runs: &[LayoutRun]) -> Option<BoundingBox> {
...
pub(crate) fn source_line_visual_bbox_for_caret(
...
pub(crate) fn source_run_visual_bbox(run: &LayoutRun) -> Option<BoundingBox> {
