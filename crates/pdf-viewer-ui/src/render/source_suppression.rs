...
use crate::viewport_culling::{bbox_intersects, styled_run_bbox};
...
impl SuppressedVectorTextRuns {
    ...
    pub(crate) fn extend(&mut self, other: SuppressedVectorTextRuns) {
        self.object_ids.extend(other.object_ids);
        self.run_indices.extend(other.run_indices);
    }

    pub(crate) fn suppressed_count_for_text_object(&self, text: &VectorTextObject) -> usize {
        text.runs
            .iter()
            .enumerate()
...
pub(crate) fn run_text_is_list_marker_only(text: &str) -> bool {
...
pub(crate) fn text_run_spatially_matches_replacement_region(
    run: &StyledRun,
...
pub(crate) fn glyph_run_spatially_matches_replacement_region(
    run: &GlyphPaintRun,
    replacement_region: &ParagraphReplacementRegion,
) -> bool {
...
pub(crate) fn text_object_matches_overlay_source_text(
...
pub(crate) fn glyph_paragraph_matches_overlay_source_text(
...
pub(crate) fn matching_text_run_refs(
    object: &VectorRenderObject,
...
pub(crate) fn text_object_should_be_suppressed(
    object: &VectorRenderObject,
    active_object_ids: &HashSet<String>,
) -> bool {
    let VectorRenderObject::Text(text) = object else {
