use pdf_viewer_core::models::{BoundingBox, VectorImageObject, VectorRenderObject};

use crate::editor::replacement_region::ParagraphReplacementRegion;
use crate::viewport_culling::{bbox_intersects, path_object_bbox};

/// Source-row decoration suppression is a render-plan concern, not DOM/editor-shell geometry.
///
/// `scene.shell_bbox` must remain tight because the host uses it to position the
/// live editor. During active editing, thin non-text PDF objects that sit on the
/// same text row must be removed from the page canvas before replacement
/// text/caret is painted. Keeping this separate prevents widening editor hit
/// boxes just to hide visual artifacts.
pub(crate) fn decorative_object_should_be_suppressed_by_overlay(
