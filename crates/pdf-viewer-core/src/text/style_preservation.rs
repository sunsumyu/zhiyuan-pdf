use crate::document::page_region_context::{
    ParagraphRegionSnapshotLine, StyleRunSnapshot, StyleSource,
};
...
pub fn distribute_text_across_runs(
    ...
    if previous_runs.len() <= 1 {
        let style = previous_runs
            .first()
            .map(|r| r.style.clone())
            .unwrap_or_else(|| fallback_style.clone());
        return reindex_style_runs(vec![make_style_run(
            &format!("{}::0", prefix),
            text,
            &style,
        )]);
    ...
    let total_prev_length = previous_runs
        .iter()
...
pub fn preserve_changed_line_styles(
    ...
    let previous_runs = previous_line
        .map(|l| l.style_runs.clone())
        .unwrap_or_default();
    let mut decorative_prefix_runs = Vec::new();
    let mut decorative_prefix_text = String::new();

    let first_body_run = previous_runs
        .iter()
        .find(|run| !is_decorative_run_text(&run.text));
    ...
    let current_has_same_decorative_prefix = decorative_prefix_char_len > 0
        ...
        && current_chars[..decorative_prefix_char_len]
            .iter()
            .collect::<String>()
            == decorative_prefix_text;

    if previous_runs.len() <= 1 {
        let fallback_style = previous_line
            .map(|l| StyleSource {
        ...
        return reindex_style_runs(vec![make_style_run(
            &format!("{}::line::{}::run::0", region_id, line_index),
            line_text,
            &style,
        )]);
    }
    ...
    let (range_start, _range_end) =
        line_selection_range(line_text, 0, selection_start, selection_end);
    ...
    if current_has_same_decorative_prefix {
        ...
        for (run_index, run) in decorative_prefix_runs.into_iter().enumerate() {
            ...
            adj_run.id = format!(
                "{}::line::{}::decorative-prefix::{}",
                region_id, line_index, run_index
            );
            if is_last {
