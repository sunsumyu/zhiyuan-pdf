...
对照 `references/naming_principles.md` 中的八条原则，逐条检查：
...
| P7 语义清晰度 | 每个核心词在项目内是否只有一个精确含义？有无抽象模糊词（plan/data/info）？ | /5 |
| P8 类型命名 | 返回类型名是否包含抽象词/动词/一词多义？函数名是否为了对齐类型名而被迫使用抽象词？ | /5 |
...
### 6. 对比优秀开源框架（Cross-Reference）

分析每个函数时，对照 `references/naming_principles.md` 中的"优秀开源框架命名模式"章节，判断该函数名是否符合业界最佳实践：

| 框架 | 代表模式 | 检查点 |
|------|---------|--------|
| **serde** | `serialize_` 前缀约定 | 是否用一致前缀表达同一类操作？ |
| **tokio** | 裸动词函数 | 是否可以用更短的裸动词替代长名？ |
| **clap** | `get_` 前缀表示查询 | 是否不必要的 `get_` 前缀？ |
| **rustc** | `is_`/`has_` 谓词 | 布尔返回函数是否缺少 `is_`/`has_`？ |
| **anyhow** | 极简函数名 | 是否过度描述？可否更短？ |
| **std** | `as_`/`to_`/`into_` 转换契约 | 转换方法是否遵循成本约定？ |
