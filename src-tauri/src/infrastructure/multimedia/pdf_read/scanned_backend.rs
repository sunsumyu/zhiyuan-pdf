use crate::infrastructure::multimedia::pdf::models::PDF_IMAGE_CACHE;
use crate::infrastructure::multimedia::pdf_read::backend::PdfReadBackend;
use crate::infrastructure::multimedia::pdf_read::types::{
...
use pdf::file::{FileOptions, CachedFile};
use pdf::object::{
...
impl Resolve,
        ...
        for kid in kids {
            let primitive = resolver
                ...
                .map_err(|e| ToString::to_string(&e))?;
            let mut dict = primitive.into_dictionary().map_err(|e| ToString::to_string(&e))?;
            let node_type = dict
...
fn open(&self, path: &str) -> Result<ReadDocumentMeta, String> {
        ...
        let decision = if page_count > 0 {
            let (width, height, resources, has_text_content) = Self::get_page_context(&file, 0)?;
            let has_font_resources = !resources.fonts.is_empty();
            let mut image_covers_page = false;
            for xref in resources.xobjects.values() {
                let obj = file.resolver().get(*xref).map_err(|e| ToString::to_string(&e))?;
                let XObject::Image(img) = &*obj else {
                    continue;
                };
...
fn get_page_preview(&self, path: &str, page_index: u16) -> Result<PagePreview, String> {
        ...
        let (width, height, resources, has_text_content) =
            Self::get_page_context(&file, page_index)?;
        ...
        for xref in resources.xobjects.values() {
            ...
            let (data, filter) = img.raw_image_data(&resolver).map_err(|e| ToString::to_string(&e))?;
