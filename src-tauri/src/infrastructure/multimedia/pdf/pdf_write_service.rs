use crate::infrastructure::multimedia::pdf::models::PdfModifications;
use crate::infrastructure::multimedia::pdf::save_engine::apply_pdf_commands;
        ...
        let working_path = {
            ...
            for patch in modifications.text_patches {
                let commands: Vec<Box<dyn PdfEditCommand>> = vec![
                    Box::new(crate::infrastructure::multimedia::pdf::commands::ReplaceTextCommand { patch })
                ];
            ...
            // 保存到工作路径
            let working_path = crate::infrastructure::multimedia::pdf::pdf_read_service::PdfReadService::get_working_path(&path);
            modified_doc
                .save(&working_path)
